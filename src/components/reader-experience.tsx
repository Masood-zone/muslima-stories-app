...
function readProgress(slug: string): SavedProgress | null {
  try {
    ...
    if (progress.version !== 1 || (progress.view !== "intro" && progress.view !== "reading") || typeof progress.offset !== "number" || !Number.isFinite(progress.offset) || progress.offset < 0) return null;
...
function saveProgress(slug: string, view: SavedProgress["view"], offset: number) {
  try {
    window.localStorage.setItem(progressKey(slug), JSON.stringify({ version: 1, view, offset } satisfies SavedProgress));
  } catch {
...
export function ReaderExperience({ story }: { story: Story }) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      setPhase("opening");
      timer = window.setTimeout(() => setPhase("intro"), window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 100 : 930);
    });
    return () => {
      ...
      <AnimatePresence>
        ...
        {phase === "intro" && (
          <motion.section key="intro" className="intro-stage wrap" initial={reduceMotion ? false : { opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: reduceMotion ? 0.01 : 0.45 }}>
            ...
            <div className="intro-content">
              ...
              <button type="button" className="primary-button begin-button" onClick={beginReading}>Begin reading <ArrowRight size={19} /></button>
        ...
        {phase === "reading" && (
          <motion.section key="reading" className="reading-stage wrap" initial={reduceMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0.01 : 0.45 }}>
            <div className="reading-heading"><div><span className="reading-eyebrow">NOW READING</span><h1>{story.title}</h1></div><button type="button" className="back-to-intro" onClick={showIntroduction}><ArrowLeft size={15} /> Story introduction</button></div>
