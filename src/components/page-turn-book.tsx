  ...
  return (
    <div className="paper">
      <div className="paper-top">
        <span>✦ MUSLIMA STORIES</span>
        <span>{title}</span>
      </div>
      <div className="paper-content" ref={innerRef}>
        {page?.blocks.map((block) => (
          <p
            key={`${block.paragraphIndex}-${block.start}`}
            className={`reading-paragraph${block.continued ? " is-continuation" : ""}`}
          >
            {block.text}
          </p>
        ))}
      ...
      <div className="paper-bottom">
        <span className="paper-ornament">✦</span>
...
export const PageTurnBook = forwardRef<PageTurnHandle, Props>(
  function PageTurnBook(
    {
      pages,
      currentIndex,
      wide,
    ...
    const rotation = useTransform(
      progress,
      (value) => (directionRef.current === 1 ? -180 : 180) * value,
    );
    const movingShadow = useTransform(
      progress,
      (value) => Math.sin(value * Math.PI) * 0.36,
    );
    const edgeLight = useTransform(
      progress,
      (value) => 0.2 + Math.sin(value * Math.PI) * 0.5,
    );
    ...
    const settle = useCallback(
      (destination: 0 | 1, direction: Direction) => {
        clearHold();
        animationRef.current?.stop();
        if (reduceMotion) {
          if (destination === 1) {
    ...
    const preview = useCallback(
      (direction: Direction) => {
        ...
        if (progress.get() < 0.08) {
          animationRef.current = animate(progress, 0.08, {
            duration: 0.22,
            ease: "easeOut",
          });
        }
    ...
    const pointerMove = (event: ReactPointerEvent<HTMLButtonElement>) => {
      ...
      const width = book
        ? book.getBoundingClientRect().width / (wide ? 2 : 1)
        : 1;
    ...
    const touchEnd = (event: ReactTouchEvent<HTMLDivElement>) => {
      ...
      const horizontal =
        (event.changedTouches[0]?.clientX ?? start.x) - start.x;
      const vertical = (event.changedTouches[0]?.clientY ?? start.y) - start.y;
      if (
        Math.abs(horizontal) > 55 &&
        Math.abs(horizontal) > Math.abs(vertical) * 1.3
      )
        turn(horizontal < 0 ? 1 : -1);
    };

    const targetIndex = currentIndex + (flip?.direction ?? 0) * (wide ? 2 : 1);
    const leftIndex =
      flip?.direction === -1 && wide ? targetIndex : currentIndex;
    const rightIndex =
      flip?.direction === 1 && wide ? targetIndex + 1 : currentIndex + 1;
      ...
      >
        ...
        <div className="book-spread">
          <div className="book-page book-page-left">
            <StoryPaper
              page={pages[wide ? leftIndex : mobileIndex]}
              pageNumber={(wide ? leftIndex : mobileIndex) + 1}
              title={title}
              innerRef={paperRef}
            />
          </div>
          ...
          {flip && !reduceMotion && (
            <motion.div
              className={`flip-leaf ${flip.direction === 1 ? "flip-forward" : "flip-backward"}`}
            ...
            >
              <div className="flip-face flip-face-front">
                ...
                <motion.div
                  ...
                  style={{ opacity: movingShadow }}
                />
                <motion.div
                  className="flip-edge"
                  style={{ opacity: edgeLight }}
                />
              </div>
              <div className="flip-face flip-face-back">
                ...
                />
                <motion.div
                  className="flip-shade flip-shade-back"
                  style={{ opacity: movingShadow }}
                />
              </div>
            </motion.div>
          ...
          {flip && !reduceMotion && (
            <motion.div
              className={`cast-shadow ${flip.direction === 1 ? "cast-shadow-left" : "cast-shadow-right"}`}
              style={{ opacity: movingShadow }}
              aria-hidden="true"
            />
          )}
          {canBack && (
            <button
              type="button"
              className="page-grab page-grab-back"
              aria-label="Hold or drag to turn to the previous page"
              aria-disabled={Boolean(flip)}
          ...
          {canNext && (
            <button
              type="button"
              className="page-grab page-grab-next"
              aria-label="Hold or drag to turn to the next page"
              aria-disabled={Boolean(flip)}
              onPointerDown={(event) => pointerDown(event, 1)}
              onPointerMove={pointerMove}
              onPointerUp={pointerUp}
